...
void find_r(fmpz *r, const fmpz *n, ulong log_2_n) {
    fmpz_t ord_r_of_n, value;
    fmpz_init(ord_r_of_n);
    fmpz_init(value);

    fmpz_ui_pow_ui(value, log_2_n, 2);
    fmpz_set_ui(r, 2);

    while (1) {
        get_order(ord_r_of_n, n, r);

        // ord_r(n) > (log n)^2
        if (fmpz_cmp(ord_r_of_n, value) > 0) {
            break;
        }
...
int is_prime_aks(const fmpz *n) {
    // 1. If (n = a^b for a ∈ N and b > 1), output COMPOSITE.
    if (is_perfect_power(n)) {
        return 0;
    }

    // 2. Find the smallest r such that ord_r(n) > (log n)^2.
    fmpz_t r;
    fmpz_init(r);
...
int main(int argc, char * argv[]) {
    ...
    if (argc == 2) {
        ...
        TIMEIT_ONCE_START
        flint_printf("%{fmpz} : %s\n", number, is_prime_aks(number) ? "PRIME" : "COMPOSITE");
        TIMEIT_ONCE_STOP
        ...
        for (fmpz_one(number); fmpz_cmp_ui(number, 1000) <= 0; fmpz_add_ui(number, number, 1)) {
            ...
            if (is_prime_aks(number) != fmpz_is_prime(number))
